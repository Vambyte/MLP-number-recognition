...
class NeuralNetwork:
    ...
    def back_propagation(self, sample):
        ...
        for i in range(len(self.weights)):
            ...
            if (i == len(self.weights)-1):
                activation = self.softmax(w_input)
            else:
    ...
    def test_network_internal(self, data):
        ...
        for sample in data:
            ...
            cost += self.log_cost(activation[np.argmax(label_data)])
    ...
    def softmax_deriv(self, inputs):
        jacobian_m = np.diag(inputs)
        for i in range(len(jacobian_m)):
            for j in range(len(jacobian_m)):
                if i == j:
                    jacobian_m[i][j] = inputs[i] * (1-inputs[i])
                else: 
                    jacobian_m[i][j] = -inputs[i]*inputs[j]
        return jacobian_m

    def log_cost(self, activation):
        return -np.log(activation)

    def log_deriv(self, activation, desired_activation):
        activation - desired_activation
...
if (train):
    ...
    net.hyper_params([784, 100, 30, 10], 0.8, 10, 100) # topologi, learning-rate, batch-size, epoker
