...
class NeuralNetwork:
    ...
    def hyper_params(self, topology = [784, 100, 10], learning_rate = 3.0, batch_size = 10, epochs = 30, regularization_param = 4.0, evaluate = True):
        ...
        for i in range(len(topology)-1):
            self.weights.append(np.random.randn(topology[i+1], topology[i])*np.sqrt(2/topology[i]))
    ...
    def start_training(self, training_data, validation_data = []):
        ...
        for epoch in range(1, self.epochs+1):
            ...
            for batch in batches:
                self.propagate_network(batch, len(training_data))
                i += 1
        ...
        if (self.evaluate):
            ...
            dirPath = "results/" + str(self.topology) + " " + str(self.learning_rate) + " " + str(self.batch_size) + " " + str(self.epochs) + " " + str(self.regularization_param)
            os.mkdir(dirPath)
    ...
    def propagate_network(self, batch, dataset_size):
        gradient_weights = []
        gradient_biases = []
        ...
        for i in range(len(self.weights)):
            self.weights[i] = (1-self.learning_rate * (self.regularization_param / dataset_size)) * self.weights[i] - (self.learning_rate / self.batch_size) * gradient_weights[i]
        for i in range(len(self.biases)):
    ...
    def back_propagation(self, sample):
        ...
        error = self.cost_deriv(activations[-1], sample[1]) * self.sigmoid_deriv(weighted_inputs[-1])
    ...
    def test_network_internal(self, data):
        ...
        for sample in data:
            ...
            cost += self.cost_mse(activation, label_data)
    ...
    def cost_deriv(self, activation, desired_activation):
        return activation - desired_activation
    
    def cost_mse(self, activation, desired_activation):
        return np.square(np.linalg.norm(desired_activation - activation)) / 2 + self.regularization_param*sum(np.linalg.norm(w)**2 for w in self.weights) / 2
...
def load_network_instance():
    ...
    network.hyper_params(params["topology"], params["learning_rate"], params["batch_size"], params["epochs"], params["regularization_param"])
...
if (train):
    ...
    net.hyper_params([784, 100, 30, 10], 0.65, 10, 100, 4.0) # topologi, learning-rate, batch-size, epoker, regularization-parametern
